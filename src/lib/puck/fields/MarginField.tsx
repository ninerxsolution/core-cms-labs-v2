...
export function MarginField({ value, onChange, label, options = PRESET_OPTIONS }: MarginFieldProps) {
  ...
  const [presetValue, setPresetValue] = useState(isPresetValue ? value : (value || "none"));
  ...
  const parseCustomValue = (val: string): { top: string; right: string; bottom: string; left: string } => {
    ...
    // ถ้าเป็น Tailwind class (เช่น "mt-4 mr-6 mb-4 ml-6")
    const twClasses = val.split(/\s+/);
    const result = { top: "", right: "", bottom: "", left: "" };
    twClasses.forEach((cls) => {
      if (cls.startsWith("mt-")) result.top = cls;
      if (cls.startsWith("mr-")) result.right = cls;
      if (cls.startsWith("mb-")) result.bottom = cls;
      if (cls.startsWith("ml-")) result.left = cls;
